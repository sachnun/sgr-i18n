...
def run_psb_decompile(
    args: list[str], repo: Path | None = None, cwd: Path | None = None
) -> None:
    require_windows()
    ...
    result = subprocess.run(
        cmd, capture_output=False, check=False, cwd=str(cwd) if cwd else None
    )
    if result.returncode != 0:
        raise RuntimeError(
            f"PsbDecompile.exe failed with exit {result.returncode}: {' '.join(cmd)}"
        )
...
def run_psbuild(json_path: Path, out_path: Path, repo: Path | None = None) -> None:
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"PsBuild.exe failed for {json_path} with exit {result.returncode}"
        )


def run_full_rebuild(
    seed: Path, staging: Path, body_out: Path, info_out: Path, key: str, level: str
) -> None:
    require_windows()
